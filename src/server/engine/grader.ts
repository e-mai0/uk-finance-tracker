...
export async function gradeDraft(
  ...
  if (ctx.firmHookDisclosed) {
    ...
    firmHookClause = `FIRM HOOK REQUIRED: this question demands at least one specific, checkable firm hook (a named recent deal, a specific desk/group/programme, a named fund or research piece) that survives the competitor-swap test. A generic, swappable "why them" fails the firm-hook criterion.`;
  } else {
